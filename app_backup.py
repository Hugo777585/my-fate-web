...
請保持客觀、精準、沉穩與同理心，用現代語言把話說清楚，給出扎實具體的命盤與流年卡點解析。
禁止提及「免費版/付費版」「我不會講」「不便透露」等字眼，不得用留一手的方式製造焦慮。
若需要引導進一步服務，只能使用以下一句話作為收尾（只寫一次即可）：
「若需針對特定月份進行更精準的逐月行運避凶、或制定一對一的深度決策布局，歡迎解鎖完整報告...」
...
👉 （點出外在壓力或變動）
👉 （點出人際/感情影響）
請指出 2～4 個最關鍵的流年卡點與它們可能帶來的現實影響（感情/工作/金錢/家宅等），並以「機率與趨勢」表達，不要恐嚇或絕對化。
...
👉 （點出現狀痛點 1）
👉 （點出現狀痛點 2）
請把「深層原因」說清楚：命盤結構如何引動心理與行為模式（例如：安全感、控制感、界線、溝通節奏），並給出可落地的調整方向。

### 💥 【卡點與決策建議】
如果這種狀況沒有調整，很容易陷入惡性循環。
請提供 3～5 點具體可執行建議（含：溝通策略、界線設置、進退節奏、情緒安放、風險控管），並以「下一步行動」結尾。
"""
