...
def get_bazi_analysis(prompt, api_key, model_name=GEMINI_MODEL):
    ...
    try:
        ...
        st.error(f"🚨 【偵測模式】發生系統攔截或模型連線異常。錯誤訊息：{e}")
        return None
...
with st.sidebar:
    ...
    # 【關鍵修正】：僅保留穩定版 gemini-1.5-flash 選項
    model_name = st.selectbox("模型版本", [GEMINI_MODEL, "gemini-1.5-pro"])
        ...
        try:
            ...
            with st.spinner(f"大師正在解析【{module_name}】..."):
                ...
                if result == "NO_API_KEY":
                    ...
                    now_tw_display = (_dt.datetime.utcnow() + _dt.timedelta(hours=8)).strftime('%Y-%m-%d %H:%M:%S')
