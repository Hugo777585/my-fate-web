...
def generate_ai_text(api_key: str, model_name: str, module_name: str, payload: dict, selected_books: list[str], is_master: bool = False) -> str:
    ...
    if not is_master:
        ...
        4. ### 【天命密碼】：用白話點出客人的性格核心優勢與潛在挑戰。
        {"5. ### 【情緣鑑定】：分析兩人的契合點、潛在摩擦點，以及如何讓感情更穩定的具體建議。" if is_pairing else "5. ### 【命運預告】：針對 2026 年提供一個具體的轉折點預告（不給具體月份與解法，引導後續諮詢）。"}
        6. ### 【用神建議】：詳細說明 [八字用神] 對客人的重要性，並給出具體的「用神補強建議」。
        7. 最後強制加上：✨ **【想解鎖完整的「破局戰術」與「運勢攻略」？】** ✨ 請截圖此畫面並點擊下方 LINE 連結預約大師！
        """
    else:
