...
<style> 
    ...
    .feature-card { background-color: #FFFFFF; border-radius: 25px; padding: 35px; text-align: center; box-shadow: 0 10px 30px rgba(0,0,0,0.05); border: 1px solid #F4F4ED; height: 100%; display: flex; flex-direction: column; justify-content: space-between; }
    .feature-icon { font-size: 50px; margin-bottom: 20px; }
    .feature-title { font-size: 26px; font-weight: 900; color: #3E3A39; margin-bottom: 15px; }
    .stButton > button { height: 56px !important; border-radius: 18px !important; font-weight: 900 !important; font-size: 19px !important; background: linear-gradient(135deg, #9A7A38, #B38E45) !important; color: white !important; border: none !important; width: 100% !important; }
...
def render_ziwei_chart(ziwei_data, user_info=None):
    ...
    ziwei_css = """
    <style>
    .ziwei-container { width: 100%; overflow-x: auto; padding: 20px 0; display: flex; justify-content: center; }
    .ziwei-grid { display: grid; grid-template-columns: repeat(4, 1fr); grid-template-rows: repeat(4, 1fr); gap: 8px; width: 100%; max-width: 650px; aspect-ratio: 1 / 1; background: #1a1a1a; border: 3px solid #d4af37; border-radius: 32px; padding: 12px; }
    .ziwei-cell { background: #2c2c2c; border: 1px solid #d4af37; padding: 12px; display: flex; flex-direction: column; position: relative; border-radius: 20px; }
