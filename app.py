...
def _ai_system_prompt(selected_books: list[str], module_name: str) -> str:
    ...
    return (
        ...
        "格式範例：\n"
        "* **八字日主（丙火）**：你就像是一盞溫暖的燈火，天生帶有熱情...\n"
        "* **命宮巨門陀羅**：這代表你心思細膩，但在處理事情時容易...\n\n"
        "【必須主動解析的五大段落】：\n"
        "1. ### 【命格本質與內心深處】：條列出核心的八字與紫微特徵，點出外表與內心的落差。\n"
        "2. ### 【事業格局與財富流向】：條列出事業星曜與五行喜忌，解析天賦與盲點。\n"
        "3. ### 【感情羈絆與姻緣課題】：條列出夫妻宮與桃花狀況，點出感情觀與考驗。\n"
        "4. ### 【大運流轉與流年吉凶】：條列出目前十年大運的重點，以及今明兩年流年的運勢起伏（該衝或該守）。\n"
        "5. ### 【大師專屬轉運處方】：給出 3 個具體能立刻執行的生活行動建議（條列式）。\n\n"
        "【輸出規範】：\n"
        "- 絕對不可使用 Markdown 表格。\n"
