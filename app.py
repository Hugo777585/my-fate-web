...
def generate_ai_text(api_key: str, model_name: str, module_name: str, payload: dict, selected_books: list[str], is_master: bool = False) -> str:
    ...
    if not is_master:
        ...
        - [目前狀態/職業]: {payload["main_person"]["person"].occupation}
        - [八字命盤]: 
        ...
        3. 請務必結合客人目前的身份背景【{payload["main_person"]["person"].occupation}】給予符合現實且具備同理心的專屬建議，避免給出不適合其身份的一般性建議。
        {"4. 若使用者身分為【八大行業】，請側重桃花人緣（如食傷生財或貪狼、廉貞等桃花星）、偏財運勢、情緒勞動與人際界線的分析，並適度給予人身安全與身心健康方面的溫暖提醒，語氣須具備高度同理心。" if payload["main_person"]["person"].occupation == "八大行業" else "4. "}
        5. ### 【天命密碼】：用白話點出客人的性格核心優勢與潛在挑戰。
        {"6. ### 【情緣鑑定】：分析兩人的契合點、潛在摩擦點，以及如何讓感情更穩定的具體建議。" if is_pairing else "6. ### 【命運預告】：針對 2026 年提供一個具體的轉折點預告（不給具體月份與解法，引導後續諮詢）。"}
        7. ### 【用神建議】：詳細說明 [八字用神] 對客人的重要性，並給出具體的「用神補強建議」。
        8. 最後強制加上：✨ **【想解鎖完整的「破局戰術」與「運勢攻略」？】** ✨ 請截圖此畫面並點擊下方 LINE 連結預約大師！
        """
    else:
