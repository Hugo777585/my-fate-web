            ...
            with st.spinner("大師正在排盤與分析中，請稍候..."):
                ...
                if is_master:
                    if analysis_mode == "【八字精論】":
                        prompt = f"""
【最高系統指令】：你现在是一个冷酷、极度专业的命理排盘系统，绝对禁止表现出聊天机器人的态度。禁止任何开场白、问候语、结语（严禁说「恩」、「哈啰」、「首先呢」等废话）。你【必须】100% 复制以下 Markdown 模板并填入数据，绝对必须使用 |---| 的 Markdown 表格语法，不可变成条列式，否则系统将会崩溃：

{base_info}
- 大师加强维度：{', '.join(advanced_params.get('focus', []))}
- 核心学理偏好：{advanced_params.get('theory', '標準')}

一、【四柱八字实盘】
| 四柱 | 天干 | 十神 | 天干 | 地支 | 藏干 | 藏干十神 |
|------|------|------|------|------|------|----------|
| 年柱 | (填入) | (填入) | (填入) | (填入) | (填入) | (填入) |
| 月柱 | (填入) | (填入) | (填入) | (填入) | (填入) | (填入) |
| 日柱日主 | (填入) | (日主) | (填入) | (填入) | (填入) | (填入) |
| 时柱 | (填入) | (填入) | (填入) | (填入) | (填入) | (填入) |
...
三、【古籍学理与大师白话指引】
(在此处直接依据《三命通会》、《滴天髓》、《穷通宝鉴》给出学理断语，并结合客人的【职业状态】给出白话建议，禁止废话。)
"""
