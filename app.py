...
🔴 警戒點二：【缺乏界線與情緒反咬】（丁火／宥瑩模組）
- 觸發條件 (八字+紫微)：1. 八字日主極弱且官殺壓迫（殺重身輕）。2. 紫微命宮逢「地空、地劫」，且夫妻宮見「巨門化權」或「巨門化忌」。
- 行為預測：內心是填不滿的情緒黑洞（空劫）。因能量太弱渴望溫暖，初期交往毫無界線。一旦感到壓力或理虧心虛，巨門星負面能量爆發，會用刻薄、顛倒是非的言語攻擊伴侶。
...
- 觸發條件 (雙人合盤)：客人盤帶強烈「庇護/照顧」特質（如丙火日主、印星極旺、紫微命宮無主星但包容力強），且詢問上述警戒點對象。
...
並在結尾溫柔地提醒客人：以上為基礎性格與運勢分析，若想進一步了解具體卡關點的「破解方案」、對象的「真實心態」或「年度行動策略」，歡迎參考下方的深度分析方案。
{tone_instruction}"""
                    elif analysis_mode == "【紫微斗數分析】":
...
並在結尾提醒客人：若需更精準的「流月避坑指引」或「PDF 完整報告」，可解鎖下方的深度分析。
{tone_instruction}"""
                    else:
...
並在結尾建議客人：交叉比對的深度細節與「具體行動指引」已整理在付費報告中，如有需要可解鎖查看。
{tone_instruction}"""
