...
- 行為預測：內心是填不滿的情緒黑洞。因能量太弱渴望溫暖，初期交往毫無界線。一旦感到壓力或理虧心虛，負面能量爆發，會用刻刻、顛倒是非的言語攻擊伴侶。
...
【🧠 心理諮商模組：深度共情與行為拆解】
當諮詢類別為「心理諮商」時，請啟動此模組：
1. 認知重構：利用心理學理論（如 CBT、阿德勒心理學）分析客人的困擾來源。
2. 情緒標記：精準識別並接納客人的情緒（如：焦慮、委屈、恐懼）。
3. 小步行動：提供 24 小時內可以執行的最小行動方案。
4. 命理與心理的交織：分析先天性格（命理）如何影響目前的心理狀態，並給予突破點。
...
with st.container():
    ...
    with form_container:
        ...
        with col_cat1:
            ...
            if st.button("💘 感情婚姻", use_container_width=True, key="main_cat_love_bak"):
                st.session_state.main_cat = "感情"
                st.session_state.form_step = 2  # 切換後進入該分類的第二步
                st.rerun()
        with col_cat2:
            if st.button("💼 事業財運", use_container_width=True, key="main_cat_job_bak"):
                st.session_state.main_cat = "事業"
                st.session_state.form_step = 2
                st.rerun()
        with col_cat3:
            if st.button("🧠 心理諮商", use_container_width=True, key="main_cat_mind_bak"):
                st.session_state.main_cat = "心理諮商"
                st.session_state.form_step = 2
                st.rerun()
        ...
        # 步驟二：次級狀態選擇
            ...
            elif st.session_state.main_cat == "事業":
                options = ["想換工作", "創業諮詢", "財運不佳", "職場人際", "升遷機會", "其他"]
            else: # 心理諮商
                options = ["情緒壓力", "人際卡關", "自我探索", "職場壓力", "原生家庭", "其他"]
        ...
        # 步驟三：細節描述
            ...
            if st.session_state.main_cat == "感情":
                placeholder_text = "描述對方具體做了什麼讓你最在意的事？"
            elif st.session_state.main_cat == "事業":
                placeholder_text = "請描述目前事業或財運上遇到的具體困難..."
            else:
                placeholder_text = "請描述您目前心理上的困擾或想探索的議題..."
...
{tone_instruction}
"""
                    elif analysis_mode == "【心理諮商分析】":
                        prompt = f"""{time_setting}您現在是一位具備心理學家與命理大師雙重身份的專家。請針對客人的心理困擾進行深度諮商分析，並結合其命盤特質：
{pillar_info}

客人資料：{base_info}

【輸出架構要求】：
1. 『 【🧠 心理狀態掃描】 』：運用心理學專業，拆解客人目前的焦慮、壓力或情緒卡點。
2. 『 【🔮 命理性格根源】 』：從八字/紫微的角度，解釋為何客人的性格特質會導致目前的心理困境。
3. 『 【🚀 認知行動指南】 』：提供具體的認知重構建議，以及未來 24 小時內可做的微小改變。
4. 『 【✨ Hugo 溫馨寄語】 』：給予深度的共情與支持。
{tone_instruction}
"""
...
{tone_instruction}"""
                        ...
                        prompt = f"""{time_setting}請以具備同理心的心理諮商師與命理師身份，分析以下情況：
{pillar_info}
客人資料：{base_info}
請劃分『 【🧠 心理掃描】 』、『 【🔮 命理性格建議】 』與『 【🚀 行動指南】 』。
{tone_instruction}"""
                    else:
